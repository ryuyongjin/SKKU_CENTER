<%@ page language="java" contentType="text/html;charset=UTF-8" pageEncoding="UTF-8" %> 
<!-- PC용 header ---------------------------------------------------------------------------------------------------------------------------------->
<div id="header">
    <!-- util -->
    <div class="topWrap">
        <div>
        <ul>
            <li><a href="/">홈</a></li>
            <c:choose>
	            <c:when test="${loginVO != null and loginVO.id != null}">
					<li>${loginVO.name }님 환영합니다.${loginVO.id}</li>
					<li><a href="${pageContext.request.contextPath}/uat/uia/actionLogout.do">로그아웃</a></li>
				</c:when>
				<c:otherwise>
		            <li><a href="#none">성균관대로그인</a></li>
		            <li><a href="/uat/uia/egovLoginUsr.do">일반인로그인</a></li>
		            <!-- <li><a href="/uss/umt/EgovMberSbscrbView.do">일반인회원가입</a></li> -->
				</c:otherwise>
            </c:choose>
        </ul>
        </div>
    </div>
    <!-- //util -->

    <!-- 네비게이션 -->
    <div class="gnbWrap">
    
    <h1><a href="/" title="메인페이지로 이동"><img src="/images/common/logo.png" alt="성균관대학교 대학교육혁신센터"></a></h1>
   
    <div class="gnb">        
        <ul>
            <li>
                <h2 class="hn1">
                 <a href="/about/greeting.do" class="globalMenu" id="menu1"><img src="/images/common/gnb01.gif" alt="센터소개" class="menuImg" /></a>
                </h2>
                <div class="sub_nav_box" id="sub_menu1">
                    <div class="sub_nav">
                        <ul>
                            <li><a href="/about/greeting.do">센터장 인사말</a></li>
                            <li><a href="/about/history.do">설립목적 및 연혁</a></li>
                            <li><a href="/about/network.do">연구 영역 네트워크</a></li>
                            <li><a href="javascript:alert('내용준비중입니다.');">공지사항</a></li>
                            <!-- 20150226 준비중 
                            <li><a href="/cop/bbs/selectBoardList.do?bbsId=BBSMSTR_000000000011">공지사항</a></li>
                            -->
                            <li><a href="/about/location.do">오시는길</a></li>
                        </ul>
                    </div>
                </div>
            </li>
            <li>
                <h2 class="hn2">
                 <a href="/feature/creative01.do" class="globalMenu" id="menu2"><img src="/images/common/gnb02.gif" alt="대학특성화" class="menuImg" /></a>
                </h2> 
                <div class="sub_nav_box" id="sub_menu2">
                    <div class="sub_nav">
                        <ul>
                            <li><a href="/feature/creative01.do">융합기반 Creative Informatics 인재양성 사업단</a></li>
                            <li><a href="/feature/convergence01.do">창의적 융복합 소재 및 공정 특성화 사업단</a></li>
                            <li><a href="/feature/engineering01.do">글로벌 건설 엔지니어링 전문인력 양성 사업단</a></li>
                            <li><a href="/feature/confucianism01.do">글로벌 유학 창의인재 양성 사업단</a></li>
                            <li><a href="/feature/ischool01.do">i-School 기반의 세계 최고 수준 데이터 전문가 양성 사업단</a></li>
                            <li><a href="/feature/physics01.do">차세대 선도 물리인재 양성 사업단</a></li>
                        </ul>
                    </div>
                </div>                   
            </li>
            <li>
                <h2 class="hn3">
                 <a href="/cschool/about01.do" class="globalMenu" id="menu3"><img src="/images/common/gnb03.gif" alt="C-School" class="menuImg" /></a>
                </h2>
                <div class="sub_nav_box" id="sub_menu3">
                    <div class="sub_nav">
                        <ul>
                            <li>
                                <a href="/cschool/about01.do">소개</a>
                                <ul class="sub_navdep2">
                                    <li><a href="/cschool/about01.do">참여학과</a></li>
                                    <li><a href="/cschool/about02.do">인재상</a></li>
                                </ul>
                            </li>
                            <li>
                                <a href="/cschool/course01.do">교육과정</a>
                                <ul class="sub_navdep2">
                                    <li><a href="/cschool/course01.do">로드맵</a></li>
                                    <li><a href="/cschool/course02.do">교과목</a></li>
                                    <li><a href="/cschool/course03.do">비교과 프로그램</a></li>
                                </ul>
                            </li>
                            <li><a href="/cschool/cadvisor.do">C-advisor 상담 예약</a></li>
                            <li><a href="/cschool/coffice.do">C-Office 소개</a></li>
                            <li>
                            	<a href="javascript:alert('내용준비중입니다.');">커뮤니티</a>
                                <ul class="sub_navdep2">
                                    <li><a href="javascript:alert('내용준비중입니다.');">NEWS</a></li>
                                    <li><a href="javascript:alert('내용준비중입니다.');">학생소감</a></li>
                                    <li><a href="javascript:alert('내용준비중입니다.');">갤러리</a></li>
                                    <li><a href="javascript:alert('내용준비중입니다.');">FAQ</a></li>
                                    <li><a href="javascript:alert('내용준비중입니다.');">Q&amp;A</a></li>
                                </ul>
                            	<!-- 20150226 준비중 
                                <a href="/cschool/news.do">커뮤니티</a>
                                <ul class="sub_navdep2">
                                    <li><a href="/cschool/news.do">NEWS</a></li>
                                    <li><a href="/cschool/student.do">학생소감</a></li>
                                    <li><a href="/cschool/gallery.do">갤러리</a></li>
                                    <li><a href="/cschool/faq.do">FAQ</a></li>
                                    <li><a href="/cschool/qa.do">Q&amp;A</a></li>
                                </ul>
                                -->
                            </li>
                        </ul>
                    </div>
                </div> 
            </li> 
            <li>
                <h2 class="hn4">
                 <a href="/ace/step_main.do" class="globalMenu" id="menu4"><img src="/images/common/gnb04.gif" alt="ACE" class="menuImg" /></a>
                </h2>
                <div class="sub_nav_box" id="sub_menu4">
                    <div class="sub_nav">
                        <ul>
                            <li>
                                <a href="/ace/step_main.do">성균 글로벌 창의리더 역량개발 : 비교과 프로그램</a>
                                <ul class="sub_navdep2">
                                    <li><a href="/ace/1step01.do">개인역량</a></li>
                                    <li><a href="/ace/2step01.do">창의·리더 역량</a></li>
                                    <li><a href="/ace/3step01.do">글로벌 창의·리더 역량</a></li>
                                </ul>
                            </li>
                            <li>
                                <a href="/ace/pro_main.do">성균 효과성 프로그램<br /> : 교육의 질 관리</a>
                                <ul class="sub_navdep2">
                                    <li><a href="/ace/pro_alc.do">Academic Life Cycle(ALC) 질관리 모델</a></li>
                                    <li><a href="/ace/pro_scca.do">성균핵심역량진단(SCCA)</a></li>
                                    <li><a href="/ace/pro_analysis.do">대학생 학습경험분석 연구</a></li>
                                </ul>
                            </li>
                            <li>
                                <a href="/ace/cl_supporters.do">성대형 ACE 모델 확산</a>
                                <ul class="sub_navdep2">
                                    <li><a href="/ace/cl_supporters.do">창의리더 서포터즈</a></li>
                                    <li><a href="/ace/cl_show.do">창의리더 공모전</a></li>
                                    <li><a href="/ace/cl_special.do">창의리더 특강</a></li>
                                    <li><a href="/ace/cl_symposium.do">창의·리더 심포지엄</a></li>
                                    <li><a href="/ace/global_challenger.do">글로벌 창조적 챌린저 프로그램</a></li>
                                    <li><a href="/ace/creativity_zone.do">창조존(Creativity Zone)</a></li>
                                    <li><a href="/ace/help_desk.do">ACE Help Desk</a></li>
                                </ul>
                            </li>
                            <li>
                            	<a href="javascript:alert('내용준비중입니다.');">커뮤니티</a>
                                <ul class="sub_navdep2">
                                    <li><a href="javascript:alert('내용준비중입니다.');">NEWS</a></li>
                                    <li><a href="javascript:alert('내용준비중입니다.');">학생소감</a></li>
                                    <li><a href="javascript:alert('내용준비중입니다.');">갤러리</a></li>
                                    <li><a href="javascript:alert('내용준비중입니다.');">FAQ</a></li>
                                    <li><a href="javascript:alert('내용준비중입니다.');">Q&amp;A</a></li>
                                </ul>
                            	<!-- 20150226 준비중 
                                <a href="/ace/news.do">커뮤니티</a>
                                <ul class="sub_navdep2">
                                    <li><a href="/ace/news.do">NEWS</a></li>
                                    <li><a href="/ace/student.do">학생소감</a></li>
                                    <li><a href="/ace/gallery.do">갤러리</a></li>
                                    <li><a href="/ace/faq.do">FAQ</a></li>
                                    <li><a href="/ace/qa.do">Q&amp;A</a></li>
                                </ul>
                                -->
                            </li>
                        </ul>
                    </div>
                </div> 
            </li>     
            <li>
            	<h2 class="hn5">
                 <a href="javascript:alert('내용준비중입니다.');" class="globalMenu" id="menu5"><img src="/images/common/gnb05.gif" alt="대학연구" class="menuImg" /></a>
                </h2>
                <div class="sub_nav_box" id="sub_menu5">
                    <div class="sub_nav">
                        <ul>
                            <li><a href="javascript:alert('내용준비중입니다.');">연구보고서 및 자료</a></li>
                            <li><a href="javascript:alert('내용준비중입니다.');">행사</a></li>
                        </ul>
                    </div>
                </div> 
            	<!-- 20150226 준비중 
                <h2 class="hn5">
                 <a href="/research/data.do" class="globalMenu" id="menu5"><img src="/images/common/gnb05.gif" alt="대학연구" class="menuImg" /></a>
                </h2>
                <div class="sub_nav_box" id="sub_menu5">
                    <div class="sub_nav">
                        <ul>
                            <li><a href="/research/data.do">연구보고서 및 자료</a></li>
                            <li><a href="/research/event.do">행사</a></li>
                        </ul>
                    </div>
                </div> 
                -->
            </li>               
            
        </ul>
    </div>
    </div>
    <!-- //네비게이션 -->

</div>
<!-- //PC용 header ---------------------------------------------------------------------------------------------------------------------------------->
    
    
    
<!-- 모바일용 header ---------------------------------------------------------------------------------------------------------------------------------->
<div id="mHeader">
    <div class="mGnb">
    	<h1><a href="/" title="메인페이지로 이동"><img src="/images/common/logo.gif" alt="성균관대학교 대학교육혁신센터"></a></h1>
    	<button type="button" class="mBtn btn_open" id="mBtnMenu"><span>전체메뉴 열기</span></button>    
    </div>
    
    <!-- 모바일 전체메뉴 start -->
    <div class="mSubmenu ">
        <nav class="allmenu">
        
        <h2 class="hide">전체메뉴</h2>
        
        <h3 class="first">센터소개</h3>
        <ul>
            <li><a href="/about/greeting.do">센터장 인사말</a></li>
            <li><a href="/about/history.do">설립목적 및 연혁</a></li>
            <li><a href="/about/network.do">연구 영역 네트워크</a></li>
            <li><a href="javascript:alert('내용준비중입니다.');">공지사항</a></li>
            <li><a href="/about/location.do">오시는길</a></li>
        </ul>
        
        <h3>대학특성화</h3>
        <ul class="width100">
            <li><a href="/feature/creative01.do">융합기반 Creative Informatics 인재양성 사업단</a></li>
            <li><a href="/feature/convergence01.do">창의적 융복합 소재 및 공정 특성화 사업단</a></li>
            <li><a href="/feature/engineering01.do">글로벌 건설 엔지니어링 전문인력 양성 사업단</a></li>
            <li><a href="/feature/confucianism01.do">글로벌 유학 창의인재 양성 사업단</a></li>
            <li><a href="/feature/ischool01.do">i-School 기반의 세계 최고 수준 데이터 전문가 양성 사업단</a></li>
            <li><a href="/feature/physics01.do">차세대 선도 물리인재 양성 사업단</a></li>
        </ul>
        
        <h3>C-School</h3>
        <dl class="menu2">
            <dt><a href="/cschool/about01.do">소개</a></dt>
            <dd>
                <ul>
                    <li><a href="/cschool/about01.do">참여학과</a></li>
                    <li><a href="/cschool/about02.do">인재상</a></li>
                </ul>
            </dd>
            <dt><a href="/cschool/course01.do">교육과정</a></dt>
            <dd>
                <ul>
                    <li><a href="/cschool/course01.do">로드맵</a></li>
                    <li><a href="/cschool/course02.do">교과목</a></li>
                    <li><a href="/cschool/course03.do">비교과 프로그램</a></li>
                </ul>
            </dd>
            <dt><a href="/cschool/cadvisor.do">C-advisor 상담 예약</a></dt>
            <dt><a href="/cschool/coffice.do">C-Office 소개</a></dt>
            <dt><a href="/">커뮤니티</a></dt>
            <dd>
                <ul>
                	<li><a href="javascript:alert('내용준비중입니다.');">NEWS</a></li>
                    <li><a href="javascript:alert('내용준비중입니다.');">학생소감</a></li>
                    <li><a href="javascript:alert('내용준비중입니다.');">갤러리</a></li>
                    <li><a href="javascript:alert('내용준비중입니다.');">FAQ</a></li>
                    <li><a href="javascript:alert('내용준비중입니다.');">Q&A</a></li>
                	<!-- 20150226 준비중
                    <li><a href="/cschool/news.do">NEWS</a></li>
                    <li><a href="/cschool/student.do">학생소감</a></li>
                    <li><a href="/cschool/gallery.do">갤러리</a></li>
                    <li><a href="/cschool/faq.do">FAQ</a></li>
                    <li><a href="/cschool/qa.do">Q&A</a></li>
                    -->
                </ul>
            </dd>
        </dl>
        
        <h3>ACE</h3>
        <dl class="menu2">
            <dt><a href="/ace/step_main.do">성균 글로벌 창의리더 역량개발 : 비교과 프로그램</a></dt>
            <dd>
                <ul>
                    <li><a href="/ace/1step01.do">개인역량</a></li>
                    <li><a href="/ace/2step01.do">창의·리더 역량</a></li>
                    <li><a href="/ace/3step01.do">글로벌 창의·리더 역량</a></li>
                </ul>
            </dd>
            <dt><a href="/ace/pro_main.do">성균 효과성 프로그램 : 교육의 질 관리</a></dt>
            <dd>
                <ul>
                    <li><a href="/ace/pro_alc.do">Academic Life Cycle(ALC) 질관리 모델</a></li>
                    <li><a href="/ace/pro_scca.do">성균핵심역량진단(SCCA)</a></li>
                    <li><a href="/ace/pro_analysis.do">학습경험분석</a></li>
                </ul>
            </dd>
            <dt><a href="/ace/cl_supporters.do">성대형 ACE 모델 확산</a></dt>
            <dd>
                <ul>
                    <li><a href="/ace/cl_supporters.do">창의리더 서포터즈</a></li>
                    <li><a href="/ace/cl_show.do">창의리더 공모전</a></li>
                    <li><a href="/ace/cl_special.do">창의리더 특강</a></li>
                    <li><a href="/ace/cl_symposium.do">창의·리더 심포지엄</a></li>
                    <li><a href="/ace/global_challenger.do">글로벌 창조적 챌린저 프로그램</a></li>
                    <li><a href="/ace/creativity_zone.do">창조존(Creativity Zone)</a></li>
                    <li><a href="/ace/help_desk.do">ACE Help Desk</a></li>
                </ul>
            </dd>
            <dt><a href="/">커뮤니티</a></dt>
            <dd>
                <ul>
                	<li><a href="javascript:alert('내용준비중입니다.');">NEWS</a></li>
                    <li><a href="javascript:alert('내용준비중입니다.');">학생소감</a></li>
                    <li><a href="javascript:alert('내용준비중입니다.');">갤러리</a></li>
                    <li><a href="javascript:alert('내용준비중입니다.');">FAQ</a></li>
                    <li><a href="javascript:alert('내용준비중입니다.');">Q&A</a></li>
                	<!-- 20150226 준비중
                    <li><a href="/ace/news.do">NEWS</a></li>
                    <li><a href="/ace/student.do">학생소감</a></li>
                    <li><a href="/ace/gallery.do">갤러리</a></li>
                    <li><a href="/ace/faq.do">FAQ</a></li>
                    <li><a href="/ace/qa.do">Q&A</a></li>
                    -->
                </ul>
            </dd>
        </dl>
        
        <h3>대학연구</h3>
        <ul>
        	<li><a href="javascript:alert('내용준비중입니다.');">연구보고서 및 자료</a></li>
            <li><a href="javascript:alert('내용준비중입니다.');">행사</a></li>
        	<!-- 20150226 준비중
            <li><a href="/research/data.do">연구보고서 및 자료</a></li>
            <li><a href="/research/event.do">행사</a></li>
            -->
        </ul>
            
        </nav>
    </div>
    <!--// 모바일 전체메뉴 end --> 
    
    <!-- util -->
    <div class="topWrap">
        <div>
        <ul>
            <li><a href="#none">성균관대로그인</a></li>
            <li><a href="#none">일반인로그인</a></li>
        </ul>
        </div>
    </div>
    <!-- //util -->
 

</div>
<!-- //모바일용 header ---------------------------------------------------------------------------------------------------------------------------------->
